...
async function updatePages(notionFetch, token, updatesArray) {
	if (!Array.isArray(updatesArray)) {
		throw new Error('updatesArray must be an array of objects with pageId and update');
	}

	const results = [];

	for (const { pageId, update } of updatesArray) {
		if (!pageId || typeof pageId !== 'string') {
			results.push({
				pageId,
				status: 'rejected',
				error: new Error(`Invalid pageId: ${pageId}`)
			});
			continue;
		}
...
async function addSelectOption(notionFetch, token, pageId, propertyName, newOption, multiSelect = true) {

	if (!pageId || typeof pageId !== 'string') {
		throw new Error(`Invalid pageId: ${pageId}`);
	}
	if (!propertyName || typeof propertyName !== 'string') {
		throw new Error(`Invalid propertyName: ${propertyName}`);
	}
	if (!newOption || typeof newOption !== 'string') {
		throw new Error(`Invalid newOption: ${newOption}`);
	}

	const propertyType = multiSelect ? 'multi_select' : 'select';
