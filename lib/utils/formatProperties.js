...
function formatProperties(userInput, schema) {
  ...
  for (const [key, value] of Object.entries(userInput)) {
    ...
    switch (prop.type) {
      ...
      case 'multi_select':
        formatted[key] = { multi_select: value.map(v => ({ name: v })) };
        break;

      case 'people':
        if (Array.isArray(value)) {
          formatted[key] = { people: value.map(id => ({ id })) };
        } else {
          formatted[key] = { people: [{ id: value }] };
        }
        break;
      ...
      case 'checkbox':
        formatted[key] = { checkbox: Boolean(value) };
        break;
      ...
      case 'relation':
        if (Array.isArray(value)) {
          formatted[key] = { relation: value.map(id => ({ id })) };
        } else {
          formatted[key] = { relation: [{ id: value }] };
        }
        break;
